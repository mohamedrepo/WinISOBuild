...
#
# Examples:
#   .\Get-WinISOUpdates.ps1 -Build 26200 -Architecture x64 -ListOnly
#   .\Get-WinISOUpdates.ps1 -Iso D:\ISO\Win11.iso
#   .\Get-WinISOUpdates.ps1 -Iso D:\ISO\Win11.iso -Kinds LatestCU,DotNetCU
...
# Build -> marketing version hints (season/year releases). Hint only.
$script:VersionHints = @(
...
function Get-WinISOBuildFromIso {
    param([string]$IsoPath)
    $result = @{ Build = 0; Architecture = '' }
    if (-not $IsoPath -or -not (Test-Path -LiteralPath $IsoPath)) { return $result }
    ...
    $img = $null
    try { $img = Mount-DiskImage -ImagePath $IsoPath -PassThru -ErrorAction Stop } catch { return $result }
    try {
        $letter = ''
        for ($i = 0; $i -lt 60; $i++) {
        ...
        $mv = [regex]::Match($out, '(?im)^\s*Version\s*:\s*(\d+)\.(\d+)\.(\d+)')
        if ($mv.Success) { $result.Build = [int]$mv.Groups[3].Value }
        $ma = [regex]::Match($out, '(?im)^\s*Architecture\s*:\s*(\w+)')
        if ($ma.Success) { $result.Architecture = [string]$ma.Groups[1].Value }
    }
    finally {
        if ($img) { try { Dismount-DiskImage -ImagePath $IsoPath -ErrorAction SilentlyContinue | Out-Null } catch { } }
    }
    return $result
}
...
function Invoke-WinISOUpdateSelfTest {
    ...
    if ((Get-WinISOVersionFromBuild 22631) -ne '23H2') { $problems += 'version map failed for 22631' }
    ...
    Write-Host 'SELFTEST OK: module found; build/version maps; search URL + download body; applicability filter.'
    exit 0
}
...
if ($Build -eq 0 -and $Iso) {
    ...
    $det = Get-WinISOBuildFromIso -IsoPath $Iso
    if ($det.Build -gt 0) { $Build = $det.Build }
    if ($det.Architecture) { $Architecture = $det.Architecture }
}
if ($Build -eq 0) {
    # fall back to the workspace inventory if present
...
Write-Host ''
Write-Host ('Applicable updates (' + $candidates.Count + '):')
foreach ($c in $candidates) { Write-Host ('  ' + [string]$c.Kb + '  [' + [string]$c.Kind + ']  ' + [string]$c.Title) }

if ($candidates.Count -eq 0) { Write-Host 'No applicable updates found (or catalog unreachable).'; exit 0 }
...
foreach ($c in $candidates) {
    ...
    foreach ($u in @($urls)) {
        ...
        if (Test-Path -LiteralPath $dest) {
            ...
            $manifestFiles.Add([pscustomobject]@{ kb = [string]$c.Kb; kind = [string]$c.Kind; url = $u; file = $name; bytes = (Get-Item $dest).Length; sha256 = (Get-FileHash -LiteralPath $dest -Algorithm SHA256).Hash })
            continue
        }
        Write-Host ('  downloading: ' + $name)
        try {
            Invoke-WebRequest -Uri $u -OutFile $dest -UseBasicParsing -UserAgent $script:UserAgent -TimeoutSec 3600
        }
        ...
        $manifestFiles.Add([pscustomobject]@{ kb = [string]$c.Kb; kind = [string]$c.Kind; url = $u; file = $name; bytes = $len; sha256 = $sha })
    }
}
